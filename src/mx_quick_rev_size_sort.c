...
static int wild_cmp_r(t_elem **ptr, int j, int pivot) {
    if (mx_strcmp(ptr[j]->size, ptr[pivot]->size) == 0)
        ...
        if (mx_strlen(ptr[j]->size) == mx_strlen(ptr[pivot]->size))
            return mx_strcmp(ptr[pivot]->size, ptr[j]->size);
...
void mx_quick_rev_size_sort(t_elem **ptr, int left, int right) {
    int i = 0;
    int j = 0;
    int pivot = 0;
    ...
    if (left < right) {
        ...
        mx_quick_rev_size_sort(ptr, left, j - 1);
        mx_quick_rev_size_sort(ptr, j + 1, right);
    }
}
